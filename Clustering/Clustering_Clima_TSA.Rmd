...
e) Plotten von den Centern der Cluster 1, 2 ,3 von 2006 bis 2020  Yay hab ich hinbekommen auch mit 2 verschiedenen Methoden 
...
g9 anwenden von einer Methode auf f :'/
...
kmeans.years <-kmeans(huss.year.mean$`apply(huss.year, 1, mean)`, centers = 3 )
...
```{r}
#Clustering für alle Jahre 

# Loop über die Jahre von 2006 bis 2020
for (year in 2006:2020) {
  
  
  # Clustering
  huss.year <- get(huss.[year])  # Annahme: Die Daten für jedes Jahr sind in den Objekten huss.2006, huss.2007, usw. gespeichert
  huss.year.mean <- as.data.frame(apply(huss.year, 1, mean))
  huss.year.mean['numbers'] <- 1:77
  kmeans.year <- kmeans(huss.year.mean$`apply(huss.year, 1, mean)`, centers = k)
  
  # Zuordnen aller originalen Werte von Cluster 1, 2, 3 zu ihrem entsprechenden Zentrum
  centers.year <- as.vector(kmeans.year$centers)
  centers.clusters.year <- data.frame(Value = kmeans.year$cluster, Center = NA)
  
  for (i in 1:length(kmeans.year$cluster)) {
    centers.clusters.year$Center[i] <- centers.year[kmeans.year$cluster[i]]
  }
  
  centers.clusters.year['numbers'] <- 1:77
  centers.clusters.year.geography <- merge(centers.clusters.year, geography, by = "numbers")
  
  # ggplot
  ggplot() +
    geom_sf(data = centers.clusters.year.geography, aes(geometry = geometry, fill = Center)) +
    coord_sf() +
    theme_minimal() +
    labs(title = paste("kmeans K = 3 humidity of Provinces", year)) +
    labs(x = "Longitude", y = "Latitude") +
  ...
  # Hier kannst du den Plot speichern, exportieren oder anzeigen lassen
  # ggsave(paste("plot_", year, ".png", sep = ""), plot = last_plot(), device = "png")
}
...
```{r}
print(centers.clusters.2006.geography)
```
