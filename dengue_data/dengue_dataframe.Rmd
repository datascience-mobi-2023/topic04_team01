...
#Datafrem in der Form der Klima daten erstellen 
```{r}
...
#Bungkan Zeile für jahre 2006 -2009 hinzufügen 
```{r}
# Vektor erstellen
vektor <- c("Bungkan", rep(0, 13))
...
#alles nochmal alphabetisch ordnen 
```{r}
#Zeilen alphabetisch sortieren 
...
#anhand der spalte Reporting_area die datendätze zusammenfügen 
...
#alle spalten in anderen kategoerien in numeric umwandeln 
```{r}
library(dplyr)

# Alle Spalten außer der ersten Spalte in numerischen Datentyp umwandeln
dengue_data_com2 <- mutate_at(dengue_data_com2, vars(-1), as.numeric)

print(dengue_data_com2)
```
#alle NA IN O umwandwandeln 

```{r}
# Werte "-" in 0 umwandeln
dengue_data_com2[is.na(dengue_data_com2)] = 0

print(dengue_data_com2)
```
#lets save the format for the dengue_data
```{r}
#locales Speichern der dengue_hum 
