...
while others reported a negative relationship. However for this dataset
...
seasons. To understand how the dataframes that are used here, were
...
### 1.1 Loading the relevant packages and the needed dataframes

The humidity and dengue case data is now sorted into their respective
monsoon seasons and stacked atop of each other. Let's load it again and
do some basic correlation tests First load the packages we need and the
data
...
left in the dataframe as they also contain the actual zero cases.
...
relationship between "Dengue" and "Humidity" in the dataframe that only
...
Because of the NA's in the dataframe, the time series needs to be
...
province and the first province is then removed and in Feburary the 3.
...
#add the NA's on top of the dengue case Data in the longformat dataframe 
dengue_lag1 <- c(lag1, dengue_lag1)
#remove the last 77 values from the vector 
dengue_lag1 <- head(dengue_lag1, -77)
...
#add the NA's on top of the dengue case Data in the longformat dataframe 
dengue_lag2 <- c(lag2, dengue_lag2)
#remove the last 77 values from the vector 
dengue_lag2 <- head(dengue_lag2, -154)
...
#add the NA's on top of the dengue case Data in the longformat dataframe 
dengue_lag3 <- c(lag3, dengue_lag3)
#remove the last 77 values from the vector 
dengue_lag3 <- head(dengue_lag3, -231)
...
variability in the dengue cases, although a the one month lag explains
about the same amount. The non-lagged correlation has a R-Squared value
of 0.05837, the one month lag has an R-Square value of 0.05547, the two
month lag has an R-Square value of 0.03035 and the three month lag has
an R-Square value of 0.002979.

## 2. Correlation between humidity and dengue sorted into monsoon seasons
...
#add the NA's on top of the dengue case Data in the longformat dataframe 
dengue_lag1_mon <- c(lag1_mon, dengue_lag1_mon)
#remove the last 77 values from the vector 
dengue_lag1_mon <- head(dengue_lag1_mon, -77)
...
The correlation between the one season lag only explains less than 1% of
the variability in the dengue cases. This is not a suitable model.
