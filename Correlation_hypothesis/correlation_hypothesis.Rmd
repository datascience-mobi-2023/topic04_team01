...
We have a stacked dataframe now, let's see how the correlation over all years looks, both lagged and not lagged 
```{r}
...
#let's look at the relationship between the monsoon humidity season and the post-monsoon dengue cases 
cor(stacked_hum_den$monH, stacked_hum_den$postmon, use = "pairwise.complete.obs")

#and at the correlation between pre-monsoon humidity and post-monsoon dengue cases
cor(stacked_hum_den$premonH, stacked_hum_den$postmon, use = "pairwise.complete.obs")

#and at the correlation between pre-monsoon humidity and monsoon dengue cases 
cor(stacked_hum_den$premonH, stacked_hum_den$mon, use = "pairwise.complete.obs")
cor(stacked_hum_den$premonH, stacked_hum_den$mon, use = "pairwise.complete.obs", method = "spearman")
```
make a scatterplot for the pre-monsoon humidity and the monsoon dengue cases 
```{r}
...
```
The most interesting correlation would be the correlation between pre-monsoon humidity and monsoon dengue cases.
The correlation does not seem great, however let's test this with a correlation test.
...
```
The correlation test returns a p-value that is smaller than 0.05 which indicates that the H0 hypothesis is rejected and the H1 hypothesis is assumed. This is not what one would expect.

If we look at the relationship between the dengue cases between the monsoon seasons the relationship is more clear 
```{r}
...
```
The H0 hypothesis can be rejected as the p-value is smaller than 0.05.
Maybe the monsoon cases could be predicted through a linear regression model with the pre-monsoon dengue cases 
```{r}
#the linear model was used before to plot an Abline in the scatterplot, we just need to summerize it
summary(model)
```
We can see that the regression model can explain 53% of the variance in this relationship as well as with a p-value smaller than 0.05 using the linear regression model here would be significantly better to predict the monsoon dengue cases than the zero model. 
